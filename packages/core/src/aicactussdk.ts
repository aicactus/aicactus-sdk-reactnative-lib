...
export module AicactusSDK {
    ...
    trackAttributionData?: boolean
		...
		/**
		 * Register a set of integrations to be used with this AicactusSDK instance.
		 */
		using?: Integration[]
		debug?: boolean
	...
	export class Client {
		 ...
		 * Previously, you would have to do this everywhere you trigger an event with the Aicactus SDK.
		 ...
		 *
		 * You'll need to call this method when you configure AicactusSDK's singleton
		 * using the native API.
		 */
		public useNativeConfiguration() {
			if(this.ready) {
				throw new Error('AicactusSDK has already been configured')
			}
		...
		/**
		 * Setup the AicactusSDK module. All calls made before are queued
		 * and only executed if the configuration was successful.
		 *
		 * ```js
		 * await aicactusSDK.setup('YOUR_WRITE_KEY', {
		 ...
		 * ```
		 *
		 * @param writeKey Your AiCactus write key
		 ...
		 * For more information on how we generate the UUID and Apple's policies on IDs
		...
		private handleError(error: Error) {
			...
			if (!handlers.length) {
				console.error('Uncaught AicactusSDK error', error)
				throw error
			} else {
