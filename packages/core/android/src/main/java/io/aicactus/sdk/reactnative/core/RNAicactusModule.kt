...
import io.aicactus.sdk.internal.Utils.getSegmentSharedPreferences
import java.util.concurrent.TimeUnit
import com.facebook.react.bridge.ReadableMap
import io.aicactus.sdk.*
import java.io.IOException
import java.net.HttpURLConnection



class RNAicactusModule(context: ReactApplicationContext): ReactContextBaseJavaModule(context) {
    override fun getName() = "RNAicactus"

    private val analytics
        get() = AicactusSDK.with(reactApplicationContext)
    ...
    fun setup(options: ReadableMap, promise: Promise) {
        ...
        if(singletonJsonConfig != null) {
            if(json == singletonJsonConfig) {
                ...
                if (BuildConfig.DEBUG) {
                    ...
                    return promise.reject("E_AICACTUS_RECONFIGURED", "AICactus SDK Client was allocated multiple times, please check your environment.")
                }
            }
        }

        val builder = AicactusSDK
        ...
        if(options.getBoolean("debug")) {
            builder.logLevel(AicactusSDK.LogLevel.VERBOSE)
        }
        ...
        try {
            AicactusSDK.setup(
                    RNAicactus.buildWithIntegrations(builder)
            )
            ...
            return promise.reject("E_AICACTUS_ERROR", e)
        }
        ...
        RNAicactus.setupCallbacks(analytics)
