...
async function prepareAndroid({
	...
	const {
		maven: {
			repo = undefined,
			name: depName = `io.aicactus.sdk.android.integrations:${slug(
				'-'
			).toLowerCase()}`,
			version = '+@aar'
		} = {},
	...
	const {
		import: factoryImport = `io.aicactus.sdk.android.integrations.${identifier}.${factoryClass}`
	} = android.factory || {}
	const classpath = `io.aicactus.sdk.reactnative.integration.${identifier}`
	...
	await Promise.all([
		...
		...['Module', 'Package'].map(name =>
			template(
				`${root}/java/io/aicactus/sdk/reactnative/integration/${name}.kt`,
...
async function prepareiOS({
	...
	const xcodeProject = 'ios/RNAicactusIntegration.xcodeproj'
	const targetXcodeProject = `ios/${nativeModule}.xcodeproj`
	const pod_name = `RNAicactusIntegration-${slug('-')}`
	const framework_name = `Aicactus-${slug()}`
	const alt_framework_name = `Aicactus_${slug()}`
	const {
		pod: {
			name: pod_dependency = `Aicactus-${slug()}`,
			...
			JSON.stringify(
				{
					...
					version: pkg.version,
					license: pkg.license,
					description: `${name} Integration for Aicactus's React-Native analytics library.`
				},
				null,
				2
			)
...
function genIntegration({ name, ios, android, npm, slug }: Integration) {
	...
	const nativeModule = `RNAicactusIntegration_${slug('_')}`
	const ctx: Context = {
		name,
		out,
		nativeModule,
